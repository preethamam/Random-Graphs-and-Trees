function G = RRTStar(startpos, endpos, boundary, iter_n, stepSize, radius, dim, obst_num, obst_r, showPlot)
%%***********************************************************************%
...
%************************************************************************%
%
% Usage: G = RRTStar(startpos, endpos, boundary, iter_n, stepSize, radius, dim, obst_num, obst_r, show_graph)
%
% Inputs:
%
...
%                                 nearest neighbor
%           radius              - Radius of the neighborhood range
...
% obst_r = 0
% show_graph = 1
% G = RRTStar(startpos, endpos, boundary, iter_n, stepSize, radius, dim, obst_num, obst_r, show_graph)
%
...
% initiate the distance of each nodes to the starting node
distance = [0];
...
for i = 1 : iter_n * 3
    ...
    % adding new node to the coordinates and record distance
    new_node_idx = size(coor,2) + 1;
    coor(:,new_node_idx) = newnode;
    dist = sqrt(sum((newnode-coor(:,IdxKDT{1}(1))).^2)) + distance(IdxKDT{1}(1));
    ...
    % Update the graph
    MdlKDT2 = KDTreeSearcher(coor');
    IdxKDT2 = rangesearch(MdlKDT2, newnode', radius);
    if length(IdxKDT2{1}) > 1
        for n = 2 : length(IdxKDT2{1})
            new_dist = sqrt(sum((newnode-coor(:,IdxKDT2{1}(n))).^2)) + dist;
            if new_dist < distance(IdxKDT2{1}(n))
                s = [s new_node_idx];
                t = [t IdxKDT2{1}(n)];
                for k = 1 : length(s)
                    if t(k) == IdxKDT2{1}(n)
...
if ~add_end
    ...
    s = [s size(coor,2)];
    t = [t size(coor,2)];
    dist = sqrt(sum((startpos'-endpos').^2));
    distance = [distance dist];
end
